...
def replicaSetArray = ["blue", "red", "green"]
...
def replicaSetName = replicaSetArray[instanceID - 1]

serviceContext.attributes.thisInstance["replicaSet"] = replicaSetName
...
if (config.sharded) {
	new AntBuilder().sequential {
		exec(executable:"${script}") {
			...
			arg line:"--port ${port}"
			arg line:"--replSet ${replicaSetName}"
		}
	}
} else {
	new AntBuilder().sequential {
